 ...
 */
$regexpP 			 = "~class=\"pagination\"(.+)</div>~isU"; 	// Пагинация
$regexpP2  		 = "~<a.*>(.+)<~isU";  
$regexpPrices  = "~<li class=\"product\">(.+)</li>~isU"; 
$regexpPrices2 = "~data-product=.*href=\"(.+)\".*>(.+)<.*class=\"price\">(.+)<~isU";
$regexpPrices3 = "~data-product=.*href=\"(.+)\".*>(.+)<~isU";
$url = 'https://megamarket-shop.com.ua/search?keyword=polaris';
  ...
  if ($info['http_code'] !== 200) {            
    $bad_urls[] = $request->url; 	
	  ...
	  foreach ($matches2 as $key) {
			...
			//print_r($matches);
			if (@$matches[1] && $matches[2] && stripos($key[0], 'В корзину') !== false) {
				$matches[1] = 'https://megamarket-shop.com.ua/'.trim($matches[1]);
