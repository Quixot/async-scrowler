 ...
 * elmaservice.com
 */
$regexpP 			 = "~id=\"pagination_blockY\"(.+)</ul>~isU"; 	// Пагинация
$regexpP2  		 = "~<a.*>(.+)<~isU";  
$regexpPrices  = "~class=\"col-tile(.+)</form>~isU"; 
$regexpPrices2 = "~ty-grid-list__item-name.*href=\"(.+)\".*title=\"(.+)\".*class=\"ty-price-num\">(.+)<~isU";
$regexpPrices3 = "~ty-grid-list__item-name.*href=\"(.+)\".*title=\"(.+)\"~isU";
$url = 'https://elmaservice.com/?match=all&subcats=Y&pcode_from_q=Y&pshort=Y&pfull=Y&pname=Y&pkeywords=Y&search_performed=Y&q=polaris&dispatch=products.search';
...
if ($qOfPaginationPages > 1) {
	...
	for ($i = 2; $i <= $qOfPaginationPages; $i++) {
		if (!in_array('https://elmaservice.com/index.php?match=all&subcats=Y&pcode_from_q=Y&pshort=Y&pfull=Y&pname=Y&pkeywords=Y&search_performed=Y&q=polaris&dispatch=products.search&page='.$i.'&search_id=64136', $already_scanned)) {
			AngryCurl::add_debug_msg('сканирую адрес: '.'https://elmaservice.com/index.php?match=all&subcats=Y&pcode_from_q=Y&pshort=Y&pfull=Y&pname=Y&pkeywords=Y&search_performed=Y&q=polaris&dispatch=products.search&page='.$i.'&search_id=64136');
	  	$AC->get('https://elmaservice.com/index.php?match=all&subcats=Y&pcode_from_q=Y&pshort=Y&pfull=Y&pname=Y&pkeywords=Y&search_performed=Y&q=polaris&dispatch=products.search&page='.$i.'&search_id=64136', null, $options);
	  	$is_scan = 1;
	  } else {
			AngryCurl::add_debug_msg('уже сканировал: '.'https://elmaservice.com/index.php?match=all&subcats=Y&pcode_from_q=Y&pshort=Y&pfull=Y&pname=Y&pkeywords=Y&search_performed=Y&q=polaris&dispatch=products.search&page='.$i.'&search_id=64136');
		}			
	}
	if ($is_scan) {
		$AC->execute(WINDOW_SIZE);
  ...
  if ($info['http_code'] !== 200) {            
    $bad_urls[] = $request->url; 	
	  ...
	  foreach ($matches2 as $key) {
			...
			//print_r($matches);
			if (@$matches[1] && $matches[2] && stripos($key[0], 'ty-btn__add-to-cart') !== false) {
