...
$regexpP2  		 = "~<a.*>(.+)<~isU";  
$regexpPrices  = "~product-item-holder(.+)</form>~isU"; 
$regexpPrices2 = "~href=\"(.+)\".*alt=\"(.+)\".*class=\"pr\">(.+)<~isU";
$regexpPrices3 = "~href=\"(.+)\".*alt=\"(.+)\"~isU";
$url = 'https://tovary.net.ua/products?keyword=polaris&page=all';
...
if ($qOfPaginationPages > 1) {
	...
	for ($i = 2; $i <= $qOfPaginationPages; $i++) {
		if (!in_array('https://tovary.net.ua/products?keyword=polaris&page='.$i, $already_scanned)) {
			AngryCurl::add_debug_msg('сканирую адрес: '.'https://tovary.net.ua/products?keyword=polaris&page='.$i);
	  	$AC->get('https://tovary.net.ua/products?keyword=polaris&page='.$i, null, $options);
	  	$is_scan = 1;
	  } else {
			AngryCurl::add_debug_msg('уже сканировал: '.'https://tovary.net.ua/products?keyword=polaris&page='.$i);
		}			
	}
	if ($is_scan) {
		$AC->execute(WINDOW_SIZE);
    ...
    $bad_urls[] = $request->url; 	
	  ...
	  foreach ($matches2 as $key) {
			...
			if (@$matches[1] && $matches[2] && stripos($key[0], 'submit') !== false) {
				$matches[1] = 'https://tovary.net.ua/'.trim($matches[1]);
