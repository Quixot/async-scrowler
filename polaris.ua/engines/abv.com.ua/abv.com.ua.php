 ...
 * abv.com.ua
 */
$regexpP 			 = "~class=\"pager\"(.+)</ul>~isU"; 	// Пагинация
$regexpP2  		 = "~<a.*>(.+)<~isU";  
$regexpPrices  = "~class=\"views-row(.+)node__content~isU"; 
$regexpPrices2 = "~<h2.*href=\"(.+)\".*>(.+)<.*class=\"display-price\">(.+)<~isU";
$regexpPrices3 = "~<h2.*href=\"(.+)\".*>(.+)<~isU";
$url = 'https://www.abv.com.ua/taxonomy/term/all?search=polaris';
...
if ($qOfPaginationPages > 1) {
	...
	for ($i = 2; $i <= $qOfPaginationPages; $i++) {
		if (!in_array('https://www.abv.com.ua/taxonomy/term/all?search=polaris&page='.$i, $already_scanned)) {
			AngryCurl::add_debug_msg('сканирую адрес: '.'https://www.abv.com.ua/taxonomy/term/all?search=polaris&page='.$i);
	  	$AC->get('https://www.abv.com.ua/taxonomy/term/all?search=polaris&page='.$i, null, $options);
	  	$is_scan = 1;
	  } else {
			AngryCurl::add_debug_msg('уже сканировал: '.'https://www.abv.com.ua/taxonomy/term/all?search=polaris&page='.$i);
		}			
	}
	if ($is_scan) {
		$AC->execute(WINDOW_SIZE);
  ...
  if ($info['http_code'] !== 200) {            
    $bad_urls[] = $request->url; 	
	  ...
	  foreach ($matches2 as $key) {
			...
			//print_r($matches);
			if (@$matches[1] && $matches[2] && stripos($key[0], 'btn-buy') !== false) {
				$matches[1] = 'https://www.abv.com.ua'.trim($matches[1]);
				
				
				$matches[2] = trim(strip_tags(str_replace('&quot;', '', $matches[2])));
	  		
	  		if (@stripos($matches[3], '.') !== false) {
	  			$matches[3] = preg_replace('~[\D]+~', '' , substr($matches[3], 0, stripos($matches[3], '.')));
	  		} else {
