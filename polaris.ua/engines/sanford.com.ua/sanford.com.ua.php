...
$regexpP2  		 = "~<a.*>(.+)<~isU";  
$regexpPrices  = "~class=\"product-layout(.+)clearfix~isU"; 
$regexpPrices2 = "~href=\"(.+)\".*title=\"(.+)\".*class=\"price\">(.+)<~isU";
$regexpPrices3 = "~href=\"(.+)\".*title=\"(.+)\"~isU";
$url = 'http://sanford.com.ua/index.php?route=product/search&search=polaris';
...
if ($qOfPaginationPages > 1) {
	...
	for ($i = 2; $i <= $qOfPaginationPages; $i++) {
		if (!in_array('http://sanford.com.ua/index.php?route=product/search&search=polaris&page='.$i, $already_scanned)) {
			AngryCurl::add_debug_msg('сканирую адрес: '.'http://sanford.com.ua/index.php?route=product/search&search=polaris&page='.$i);
	  	$AC->get('http://sanford.com.ua/index.php?route=product/search&search=polaris&page='.$i, null, $options);
	  	$is_scan = 1;
	  } else {
			AngryCurl::add_debug_msg('уже сканировал: '.'http://sanford.com.ua/index.php?route=product/search&search=polaris&page='.$i);
		}			
	}
	if ($is_scan) {
		$AC->execute(WINDOW_SIZE);
  ...
  if ($info['http_code'] !== 200) {            
    $bad_urls[] = $request->url; 	
	  ...
	  foreach ($matches2 as $key) {
			...
			//print_r($matches);
			if (@$matches[1] && $matches[2] && stripos($key[0], 'fa-shopping-cart') !== false) {
				
				$matches[1] = trim(str_replace('&amp;', '&', $matches[1]));
				$matches[1] = trim(htmlspecialchars_decode($matches[1]));
				
				if (stripos($matches[1], 'search=polaris') !== false) {
					$matches[1] = substr($matches[1], 0, stripos($matches[1], 'search=polaris'));
				}
