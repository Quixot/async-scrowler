 ...
 * bitovoy.com.ua
 */
$regexpP 			 = "~<ul class=\"pagination(.+)</ul>~isU"; 	// Пагинация
$regexpP2  		 = "~<a.*>(.+)<~isU";  
$regexpPrices  = "~product_card--shadow(.+)</form>~isU"; 
$regexpPrices2 = "~product_card-title.*href=\"(.+)\".*>(.+)<.*class=\"product_card-price\">(.+)<~isU";
$regexpPrices3 = "~product_card-title.*href=\"(.+)\".*>(.+)<~isU";
$url = 'https://www.bitovoy.com.ua/search?q=polaris';
...
if ($qOfPaginationPages > 1) {
	...
	for ($i = 2; $i <= $qOfPaginationPages; $i++) {
		if (!in_array('https://www.bitovoy.com.ua/search?page='.$i.'&q=polaris', $already_scanned)) {
			AngryCurl::add_debug_msg('сканирую адрес: '.'https://www.bitovoy.com.ua/search?page='.$i.'&q=polaris');
	  	$AC->get('https://www.bitovoy.com.ua/search?page='.$i.'&q=polaris', null, $options);
	  	$is_scan = 1;
	  } else {
			AngryCurl::add_debug_msg('уже сканировал: '.'https://www.bitovoy.com.ua/search?page='.$i.'&q=polaris');
		}			
	}
	if ($is_scan) {
		$AC->execute(WINDOW_SIZE);
  ...
  if ($info['http_code'] !== 200) {            
    $bad_urls[] = $request->url; 	
	  ...
	  foreach ($matches2 as $key) {
			...
			//print_r($matches);
			if (@$matches[1] && $matches[2] && stripos($key[0], 'submit') !== false) {
				$matches[1] = 'https://www.bitovoy.com.ua'.trim($matches[1]);
