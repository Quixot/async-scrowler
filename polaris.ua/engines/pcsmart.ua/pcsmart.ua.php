...
$regexpP2  		 = "~<a.*>(.+)<~isU";  
$regexpPrices  = "~class=\"product-layout(.+)product-info~isU"; 
$regexpPrices2 = "~class=\"image.*href=\"(.+)\".*title=\"(.+)\".*class=\"price\">(.+)<~isU";
$regexpPrices3 = "~class=\"image.*href=\"(.+)\".*title=\"(.+)\"~isU";
$url = 'https://pcsmart.ua/index.php?route=product/search&search=polaris';
...
if ($qOfPaginationPages > 1) {
	...
	for ($i = 2; $i <= $qOfPaginationPages; $i++) {
		if (!in_array('https://pcsmart.ua/index.php?route=product/search&search=polaris&page='.$i, $already_scanned)) {
			AngryCurl::add_debug_msg('сканирую адрес: '.'https://pcsmart.ua/index.php?route=product/search&search=polaris&page='.$i);
	  	$AC->get('https://pcsmart.ua/index.php?route=product/search&search=polaris&page='.$i, null, $options);
	  	$is_scan = 1;
	  } else {
			AngryCurl::add_debug_msg('уже сканировал: '.'https://pcsmart.ua/index.php?route=product/search&search=polaris&page='.$i);
		}			
	}
	if ($is_scan) {
		$AC->execute(WINDOW_SIZE);
