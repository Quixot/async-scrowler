 ...
 * tt.ua
 */
$regexpP 			 = "~class=\"pagination bottom\">(.+)</div>~isU";
$regexpP2 		 = "~<a.*>(.+)</a>~isU";
$regexpPrices  = "~class=\"item_inner(.+)product_icons~isU";
$regexpPrices2 = "~class=\"image\".*href=\"(.+)\".*title=\"(.+)\".*class=\"price.*\">(.+)<~isU";
$regexpPrices3 = "~class=\"image\".*href=\"(.+)\".*title=\"(.+)\"~isU";
$url = 'https://tt.ua/search?keyword=polaris';
...
if ($qOfPaginationPages > 1) { 
	echo 'Количество страниц: '.$qOfPaginationPages.PHP_EOL;
    ...
    $bad_urls[] = $request->url; 	
  	...
  	foreach ($matches2 as $key) {
  		//if (strripos($key[0], 'Нет в наличии') === false) {
				...
				//print_r($matches);
				if (@$matches[1] && $matches[2] && stripos($key[0], 'gaAddToCart') !== false) {
