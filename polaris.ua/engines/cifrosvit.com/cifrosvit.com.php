 ...
 * cifrosvit.com
 */
$regexpP 			 = "~pagination-block(.+)</table>~isU"; 	// Пагинация
$regexpP2  		 = "~<a.*>(.+)<~isU";  
$regexpPrices  = "~class=\"tovar(.+)wishlist-selected~isU"; 
$regexpPrices2 = "~class=\"name-tovar.*href=\"(.+)\".*>(.+)<.*class=\"priceUA\">(.+)<~isU";
$regexpPrices3 = "~class=\"name-tovar.*href=\"(.+)\".*>(.+)<~isU";
$url = 'https://cifrosvit.com/catalog/search/result/polaris/';
...
if ($qOfPaginationPages > 1) {
	...
	for ($i = 2; $i <= $qOfPaginationPages; $i++) {
		if (!in_array('https://cifrosvit.com/catalog/search/result/polaris/page'.$i.'/', $already_scanned)) {
			AngryCurl::add_debug_msg('сканирую адрес: '.'https://cifrosvit.com/catalog/search/result/polaris/page'.$i.'/');
	  	$AC->get('https://cifrosvit.com/catalog/search/result/polaris/page'.$i.'/', null, $options);
	  	$is_scan = 1;
	  } else {
			AngryCurl::add_debug_msg('уже сканировал: '.'https://cifrosvit.com/catalog/search/result/polaris/page'.$i.'/');
		}			
	}
	if ($is_scan) {
		$AC->execute(WINDOW_SIZE);
  ...
  if ($info['http_code'] !== 200) {            
    $bad_urls[] = $request->url; 	
	  ...
	  foreach ($matches2 as $key) {
			...
			//print_r($matches);
			if (@$matches[1] && $matches[2] && stripos($key[0], 'В наличии') !== false) {
				$matches[1] = 'https://cifrosvit.com'.trim($matches[1]);
