...
$regexpP2  		 = "~<a.*>(.+)<~isU";  
$regexpPrices  = "~product-layoutcatnew(.+)class=\"quickview~isU"; 
$regexpPrices2 = "~<h4.*href=\"(.+)\".*>(.+)<.*class=\"price price_pricelist\">(.+)<~isU";
$regexpPrices3 = "~class=\"product-name.*href=\"(.+)\".*title=\"(.+)\"~isU";
$url = 'https://intelect.com.ua/search/?search=polaris';
...
if ($qOfPaginationPages > 1) {
	...
	for ($i = 2; $i <= $qOfPaginationPages; $i++) {
		if (!in_array('https://intelect.com.ua/search/page-'.$i.'?search=polaris', $already_scanned)) {
			AngryCurl::add_debug_msg('сканирую адрес: '.'https://intelect.com.ua/search/page-'.$i.'?search=polaris');
	  	$AC->get('https://intelect.com.ua/search/page-'.$i.'?search=polaris', null, $options);
	  	$is_scan = 1;
	  } else {
			AngryCurl::add_debug_msg('уже сканировал: '.'https://intelect.com.ua/search/page-'.$i.'?search=polaris');
		}			
	}
	if ($is_scan) {
		$AC->execute(WINDOW_SIZE);
...
function callback_one($response, $info, $request) {
  ...
  global $bad_urls;
	...
	//echo $response;
	//file_put_contents('/var/www/polaris.ua/engines/intelect.com.ua/'.time(), $response);
