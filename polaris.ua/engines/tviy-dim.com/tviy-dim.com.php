 ...
 * tviy-dim.com
 */
$regexpP 			 = "~class=\"pager__list\"(.+)</ul>~isU"; 	// Пагинация
$regexpP2  		 = "~<a.*span.*>(.+)<~isU";  
$regexpPrices  = "~class=\"page-catalog__prod\">(.+)page-catalog__prod-col3.*<p.*<p.*</div.*</div~isU"; 
$regexpPrices2 = "~href=\"(.+)\".*>(.+)<.*class=\"page-catalog__prod-price\">(.+)</span~isU";
$regexpPrices3 = "~href=\"(.+)\".*>(.+)<~isU";
$url = 'https://www.tviy-dim.com/search/search/polaris.html';
...
if ($qOfPaginationPages > 1) {
	...
	for ($i = 2; $i <= $qOfPaginationPages; $i++) {
		if (!in_array('https://www.tviy-dim.com/search/search/polaris/p/'.$i.'.html', $already_scanned)) {
			AngryCurl::add_debug_msg('сканирую адрес: '.'https://www.tviy-dim.com/search/search/polaris/p/'.$i.'.html');
	  	$AC->get('https://www.tviy-dim.com/search/search/polaris/p/'.$i.'.html', null, $options);
	  	$is_scan = 1;
	  } else {
			AngryCurl::add_debug_msg('уже сканировал: '.'https://www.tviy-dim.com/search/search/polaris/p/'.$i.'.html');
		}			
	}
	if ($is_scan) {
		$AC->execute(WINDOW_SIZE);
  ...
  if ($info['http_code'] !== 200) {            
    $bad_urls[] = $request->url; 	
	  ...
	  foreach ($matches2 as $key) {
			...
			//print_r($matches);
			if (@$matches[1] && $matches[2] && stripos($key[0], 'page-catalog__prod-buy') !== false) {
				$matches[1] = trim($matches[1]);
				
				
				$matches[2] = trim(strip_tags(iconv('windows-1251', 'utf-8', $matches[2])));
