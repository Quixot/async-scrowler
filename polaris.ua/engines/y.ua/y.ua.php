 ...
 * y.ua
 */
$regexpP 			 = "~class=\"subtitle-block\">(.+)</div>~isU"; 	// Пагинация
//$regexpP2  		 = "~<a.*>(.+)<~isU";  
$regexpPrices  = "~class=\"products-item(.+)</div></div></div></div></div>~isU"; 
$regexpPrices2 = "~class=\"name-block\".*href=\"(.+)\".*title=\"(.+)\".*class=\"regular\">(.+)<~isU";
$regexpPrices3 = "~class=\"name-block\".*href=\"(.+)\".*title=\"(.+)\"~isU";
...
$AC->get('https://y.ua/search?q=polaris', null, $options);
...
if ($qOfPaginationPages > 1) {
	...
	for ($i = 2; $i <= $qOfPaginationPages; $i++) {
		if (!in_array('https://y.ua/search?p='.$i.';q=polaris', $already_scanned)) {
			AngryCurl::add_debug_msg('сканирую адрес: '.'https://y.ua/search?p='.$i.';q=polaris');
	  	$AC->get('https://y.ua/search?p='.$i.';q=polaris', null, $options);
	  	$is_scan = 1;
	  } else {
			AngryCurl::add_debug_msg('уже сканировал: '.'https://y.ua/search?p='.$i.';q=polaris');
		}			
	}
	if ($is_scan) {
		$AC->execute(WINDOW_SIZE);
    ...
    $bad_urls[] = $request->url; 	
	  ...
	  foreach ($matches2 as $key) {
			...
			if (@$matches[1] && $matches[2] && stripos($key[0], 'Купить') !== false) {
				$matches[1] = 'https://y.ua'.trim($matches[1]);
