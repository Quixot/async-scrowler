 ...
 * zhuk.ua
 */
$regexpP 			 = "~class=\"pager__container\"(.+)</div>~isU"; 	// Пагинация
$regexpP2  		 = "~<a.*span.*>(.+)<~isU";  
$regexpPrices  = "~<li class=\"catalog-grid__item(.+)</li~isU"; 
$regexpPrices2 = "~catalogCard-title.*href='(.+)'.*title=\"(.+)\".*class=\"catalogCard-price\">(.+)<~isU";
$regexpPrices3 = "~catalogCard-title.*href='(.+)'.*title=\"(.+)\"~isU";
$url = 'https://zhuk.ua/catalog/search/?q=polaris';
...
$url = 'https://fs.com.ua/ua/Lekarstvennye-sredstva/68Pri-narushenii-raboty-immunnoj-sistemy';
...
if ($qOfPaginationPages > 1) {
	...
	for ($i = 2; $i <= $qOfPaginationPages; $i++) {
		if (!in_array('https://zhuk.ua/catalog/search/filter/page='.$i.'/?q=polaris', $already_scanned)) {
			AngryCurl::add_debug_msg('сканирую адрес: '.'https://zhuk.ua/catalog/search/filter/page='.$i.'/?q=polaris');
	  	$AC->get('https://zhuk.ua/catalog/search/filter/page='.$i.'/?q=polaris', null, $options);
	  	$is_scan = 1;
	  } else {
			AngryCurl::add_debug_msg('уже сканировал: '.'https://zhuk.ua/catalog/search/filter/page='.$i.'/?q=polaris');
		}			
	}
	if ($is_scan) {
		$AC->execute(WINDOW_SIZE);
...
function callback_one($response, $info, $request) {
  ...
  global $bad_urls;
	...
	echo $response;
	file_put_contents('/var/www/polaris.ua/engines/apt.ua/cont.txt', $response);
	die();
  ...
  if ($info['http_code'] !== 200) {            
    $bad_urls[] = $request->url; 	
	  ...
	  foreach ($matches2 as $key) {
			...
			//print_r($matches);
			if (@$matches[1] && $matches[2] && stripos($key[0], 'j-buy-button-add') !== false) {
				$matches[1] = 'https://zhuk.ua/'.trim($matches[1]);
