...
$regexpPrices  = "~class=\"product\".*>(.+)class=\"price-buttons-catalog(.+)</div>~isU";
$regexpPrices2 = "~class=\"item-name\".*>.*<a.*href=\"(.+)\".*>(.+)</a>.*data-of=\"price-total\".*>(.+)<~isU";
...
function regular_one() {
	 ...
	 * Блок выбора прокси
	 */
	$cmd = 'timeout -k 115s 120s casperjs /var/www/engines/frau-technica.ru/casper.js '.escapeshellarg(ENGINE_TYPE).' '.escapeshellarg($useragent).' '.$matches_proxy[1].' '.$matches_proxy[2].' '.$matches_proxy[3].' '.$matches_proxy[4].' '.escapeshellarg($region);
	echo 'request: '.$cmd.PHP_EOL;
	$response = exec($cmd, $out, $err);
	$response = stripcslashes($response);

	//file_put_contents('/var/www/engines/frau-technica.ru/content.txt', $response);
	...
	if ($response) {
		...
		foreach ($matches2 as $key) {
			...
			//print_r($matches);
			if ($matches[1]) {
				if (strripos($key[2], 'class="btn btn-cart"') !== false) {
					price_change_detect('http://' . ENGINE_CURR . trim($matches[1]), trim($matches[2]), preg_replace('~[\D]+~', '' , $matches[3]), date("d.m.y-H:i:s"), $matches_proxy[1].':'.$matches_proxy[2], $matches_proxy[3].':'.$matches_proxy[4], $useragent, ENGINE_CURR, ENGINE_TYPE);
					$itemsArray['http://' . ENGINE_CURR . trim($matches[1])] = array(trim($matches[2]), preg_replace('~[\D]+~', '' , $matches[3]), date("d.m.y-H:i:s"), $matches_proxy[1].':'.$matches_proxy[2]); //iconv('UTF-8', 'Windows-1251', 
					echo trim($matches[2]).' | '.preg_replace('~[\D]+~', '' , $matches[3]);
				} else {
					price_change_detect('http://' . ENGINE_CURR . trim($matches[1]), trim($matches[2]), '0', date("d.m.y-H:i:s"), $matches_proxy[1].':'.$matches_proxy[2], $matches_proxy[3].':'.$matches_proxy[4], $useragent, ENGINE_CURR, ENGINE_TYPE);
					$itemsArray['http://' . ENGINE_CURR . trim($matches[1])] = array(trim($matches[2]), '0', date("d.m.y-H:i:s"), $matches_proxy[1].':'.$matches_proxy[2]); //iconv('UTF-8', 'Windows-1251', 
