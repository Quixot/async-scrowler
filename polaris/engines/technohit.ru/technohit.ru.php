...
switch (EXTRA_PARAM) {
  ...
  default:
	...
	$regexpP2 = "~<a.*>(.+)<~isU";
	$regexpPrices = "~class=\"product-short\">(.+)</form>~isU";
	...
	if (EXTRA_PARAM == 'spb') {
		...
		$regexpP1	= "~id=\"pager\"(.*)</div>~isU";
		$regexpP2 = "~<a.*>(.+)<~isU";
		$regexpPrices = "~class=\"product\"(.+)class=\"actions\"~isU";
...
print_r($matches2);
	  	foreach ($matches2 as $key) {
	  		if (strripos($key[1], 'name="BUY"') !== false) {
...
//print_r($matches);
		      ...
		      $matches[2] = trim($matches[2]);
					if (stripos(iconv('windows-1251', 'utf-8', $key[1]), 'Товар в наличии') !== false) {
						price_change_detect(trim($matches[1]), $matches[2], preg_replace('~[\D]+~', '' , $matches[3]), date("d.m.y-H:i:s"), $request->options[10004], $request->options[10006], $request->options[10018], ENGINE_CURR, ENGINE_TYPE);
						$itemsArray[trim($matches[1])] = array(
							$matches[2], 
							preg_replace('~[\D]+~', '' , $matches[3]),
