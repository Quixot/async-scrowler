...
$reg1 = "~class=\"item-view(.+)class=\"full-cover-link~isU";
...
function regular_one() {
	...
	if (ENGINE_LOOP == 2) {
		...
		$cmd = 'timeout -k 300s 301s casperjs /var/www/polaris/engines/ozon.ru/ozon.js '.escapeshellarg($url).' '.escapeshellarg($useragent).' '.$matches_proxy[1].' '.$matches_proxy[2].' '.$matches_proxy[3].' '.$matches_proxy[4].' '."'".$city."'";
		echo 'request: '.$cmd.PHP_EOL;
		$response = exec($cmd, $out, $err);
		$response = stripcslashes($response);

		//file_put_contents('/var/www/polaris/engines/ozon.ru/content.txt', $response);
