...
$reg1 = "~style=\"grid-column-start(.+)</div></div></a></div>~isU";
$reg2 = "~href=\"(.+)\".*data-test-id=\"tile-name\".*>(.+)<.*data-test-id=\"tile-price.*>(.+)<~isU";
...
//$reg3 = "~<html(.+)</html~isU";
//$reg4 = "~href=\"(.+)\".*data-test-id=\"tile-price.*>(.+)<.*data-test-id=\"tile-name\".*>(.+)<~isU";
...
$itemsArray = array();
	...
	$AC->init_console();
	callback_three($response, $info, $request);
...
//die();
		...
		if (strlen($response) > 500) {
		 ...
		 //file_put_contents('/var/www/polaris/engines/ozon.ru/content.txt', print_r($matches2, 1));die();
		  //print_r($matches2);
		  ...
		  foreach ($matches2 as $key => $value) {
		  	//echo $value[0].PHP_EOL.PHP_EOL;die();

				//$reg2 = "~class=\"main\".*>(.+)<.*href=\"(.+)\".*name-link\".*>(.+)<~isU";
				preg_match($reg2, $value[0], $matches);
				//print_r($matches);die();
				
				if (@$matches[1] && $matches[2] && stripos($value[0], 'notify') === false && strripos($value[0], 'Нет в наличии') === false && strripos($value[0], 'Узнать о поступлении') === false) {
					...
					$address = 'https://www.ozon.ru'.$matches[2];
					...
					$itemsArray[$address] = array($name, $price, date("d.m.y-H:i:s"), 'manual:manual');
					...
					$price = 0;
					price_change_detect($address, $name, $price, date("d.m.y-H:i:s"),'manual', 'manual', 'manual', ENGINE_CURR, ENGINE_TYPE);
					$itemsArray[$address] = array($name, $price, date("d.m.y-H:i:s"),'manual');
					echo $name.' | '.$price.PHP_EOL;			

					//file_put_contents(AC_DIR . DIRECTORY_SEPARATOR . 'engines' . DIRECTORY_SEPARATOR . ENGINE_CURR . DIRECTORY_SEPARATOR . 'data' . DIRECTORY_SEPARATOR . date("d.m.y") . '_' . ENGINE_CURR . '_' . ENGINE_TYPE . '_' . EXTRA_PARAM . '.data', serialize($itemsArray));		
				}
			}
			echo '<h1>Общее к-во: '.$count_all_items.'</h1>'.PHP_EOL;
		  //file_put_contents(AC_DIR . DIRECTORY_SEPARATOR . 'engines' . DIRECTORY_SEPARATOR . ENGINE_CURR . DIRECTORY_SEPARATOR . 'data' . DIRECTORY_SEPARATOR . date("d.m.y") . '_' . ENGINE_CURR . '_' . ENGINE_TYPE . '_' . EXTRA_PARAM . '.data', serialize($itemsArray));
	...
	function callback_three($response) {
	global $reg1;
	global $reg2;
	...
	global $city;
					...
					preg_match_all("~isAdult.*&quot;link&quot;:&quot;(.+)?_bctx.*&quot;title&quot;:&quot;(.+)&quot;,.*&quot;availability&quot;:(.+),&quot;.*finalPrice&quot;:(.+),~isU", $response, $matches2, PREG_SET_ORDER);
				  ...
				  foreach ($matches2 as $key => $matches) {
						...
						if ($matches[3] == '1') {
							...
							if (stripos($name, 'Кухонные весы Polaris PKS 0832DG Raspberry') === false) { // Костыль, убирает завышенную цену
								price_change_detect($address, $name, $price, date("d.m.y-H:i:s"), 'manual', 'manual', 'manual', ENGINE_CURR, ENGINE_TYPE);
								$itemsArray[$address] = array($name, $price, date("d.m.y-H:i:s"), 'manual', 'manual', 'manual');
								echo $address.' | '.$name.' | '.$price.PHP_EOL;
								$count_all_items++;
								//file_put_contents(AC_DIR . DIRECTORY_SEPARATOR . 'engines' . DIRECTORY_SEPARATOR . ENGINE_CURR . DIRECTORY_SEPARATOR . 'data' . DIRECTORY_SEPARATOR . date("d.m.y") . '_' . ENGINE_CURR . '_' . ENGINE_TYPE . '_' . EXTRA_PARAM . '.data', serialize($itemsArray));
							}
