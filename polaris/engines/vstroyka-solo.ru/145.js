...
casper.start('https://vstroyka-solo.ru/', function() {
	...
	this.then(function() {
		this.waitForSelector('#listView', function () {
			...
			this.wait(6000);
			this.click(x('//*[@div.class="pagination">li and text()="4"]'));
			this.waitForSelector('#listView', function () {
				context = context+this.getHTML('#listView');
				this.echo('TRYING TO SAVE PAGE: # 4 !!!');
				this.echo(JSON.stringify(this.getHTML('#listView')));

				this.wait(6000);
				this.click(x('//*[@div.class="pagination">li and text()="5"]'));
				this.waitForSelector('#listView', function () {
					context = context+this.getHTML('#listView');  // 3
					this.echo('TRYING TO SAVE PAGE: # 5 !!!');
					//this.echo(JSON.stringify(this.getHTML('#listView')));
					//this.wait(20000);
					//fs.write('/var/www/polaris/engines/vstroyka-solo.ru/content/145.txt', context, 'w');									
