...
casper.start('https://vstroyka-solo.ru/', function() {
	...
	this.then(function() {
		this.waitForSelector('#listView', function () {
			...
			this.wait(2000);
			this.click(x('//*[@div.class="pagination">li and text()="8"]'));
			this.waitForSelector('#listView', function () {
				context = context+this.getHTML('#listView');
				this.echo('TRYING TO SAVE PAGE: # 8 !!!');
				//this.echo(JSON.stringify(this.getHTML('#listView')));
				...
				this.waitForSelector('#listView', function () {
					...
					this.echo('TRYING TO SAVE PAGE: # 9 !!!');
					//this.echo(JSON.stringify(this.getHTML('#listView')));

					//this.wait(20000);
					//fs.write('/var/www/polaris/engines/vstroyka-solo.ru/content/189.txt', context, 'w');									
