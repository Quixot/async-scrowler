...
casper.start('https://vstroyka-solo.ru/', function() {
	...
	this.then(function() {
		this.waitForSelector('#listView', function () {
			...
			this.wait(2000);
			this.click(x('//*[@div.class="pagination">li and text()="6"]'));
			this.waitForSelector('#listView', function () {
				context = context+this.getHTML('#listView');
				this.echo('TRYING TO SAVE PAGE: # 6 !!!');
				//this.echo(JSON.stringify(this.getHTML('#listView')));

				this.wait(2000);
				this.click(x('//*[@div.class="pagination">li and text()="7"]'));
				this.waitForSelector('#listView', function () {
					context = context+this.getHTML('#listView');  // 3
					this.echo('TRYING TO SAVE PAGE: # 7 !!!');
					//this.echo(JSON.stringify(this.getHTML('#listView')));

					//this.wait(20000);
					//fs.write('/var/www/polaris/engines/vstroyka-solo.ru/content/167.txt', context, 'w');									
