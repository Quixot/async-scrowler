...
// Города - Миллионики
  ...
  case 'novosibirsk': $region = 'CityCZ_2246'; $region_name = 'Новосибирск'; break;
	...
	$regexpPrices_alt = "~class=\"product-tile sel-product-tile(.+)<div class=\"product-tile-gift-card\">~isU"; // Все товары на странице
	...
	function callback_two($response) {
		...
		if (strlen($response) > 500) {
	  	...
	  	if (stripos($region_name, trim($region_m[1])) === false ) {
		  ...
		  foreach ($matches2 as $key) {
				...
				if (@$matches[1]) {
					...
					if (strripos($key[1], 'data-pushable="true') !== false) {
						price_change_detect($matches[1], $matches[2], $matches[3], date("d.m.y-H:i:s"), 'manual', 'manual', 'manual', ENGINE_CURR, ENGINE_TYPE);
						$itemsArray[$matches[1]] = array($matches[2], $matches[3], date("d.m.y-H:i:s"), 'manual', 'manual', 'maunal');		
						...
						$itemsArray['http://' . ENGINE_CURR . trim($matches[1])] = array(trim($matches[2]), '0', date("d.m.y-H:i:s"), 'manual', 'manual', 'maunal');		
					...
					if (@$matches[1]) {
						...
						price_change_detect($matches[1], $matches[2], '0', date("d.m.y-H:i:s"), 'manual', 'manual', 'manual', ENGINE_CURR, ENGINE_TYPE);
						$itemsArray[$matches[1]] = array($matches[2], '0', date("d.m.y-H:i:s"), 'manual', 'manual', 'maunal');		
