...
$options 			= array(
	...
	CURLOPT_COOKIEJAR => '/var/www/polaris/engines/mvideo.ru/xxx/1.txt',
        				CURLOPT_REFERER         => 'http://www.mvideo.ru',
...
for ($iGlobalLoop = 0; $iGlobalLoop < ENGINE_LOOP; $iGlobalLoop++) {
	...
	$AC->get('http://www.mvideo.ru/product-list-page?q=polaris&region_id=1&limit=12&_requestid=658522', NULL, $options);
/*
		  ...
		  foreach ($matches2 as $key) {
				...
				if (@$matches[1]) {
					if (strripos($key[1], 'submit-basket') !== false) {
						...
						$itemsArray['http://' . ENGINE_CURR . trim($matches[1])] = array(trim($matches[2]), '0',
						date("d.m.y-H:i:s"),
						$request->options[10004],
						$request->options[10018],
						$request->url);		
      ...
      $bad_urls[] = $info['url'];    
		  ...
		  foreach ($matches2 as $key) {
				...
				if (@$matches[1]) {
					if (strripos($key[1], 'submit-basket') !== false) {
						price_change_detect('http://' . ENGINE_CURR . trim($matches[1]), trim($matches[2]), trim($matches[3]), date("d.m.y-H:i:s"), $request->options[10004], $request->options[10006], $request->options[10018], ENGINE_CURR, ENGINE_TYPE);
						$itemsArray['http://' . ENGINE_CURR . trim($matches[1])] = array(trim($matches[2]), trim($matches[3]),
						date("d.m.y-H:i:s"),
						$request->options[10004],
						$request->options[10018],
						$request->url);		
