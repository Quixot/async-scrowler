 ...
 */
	...
	foreach ($pathes as $key) { // Перебираем каждый файлик
		$STORE = substr($key, 0, stripos($key, '_'));
		$STORE = substr($STORE, strripos($STORE, '/')+1);
		$data_file = str_replace('.xlsx', '.data', substr($key, strripos($key, $STORE)));
		$file_path 	= '/var/www/'.ENGINE_TYPE.'/engines/'.$STORE.'/data/'.$cur_date .'_'.$data_file;
		$CITY = substr($data_file, strripos($data_file, '_')+1);
		$CITY = substr($CITY, 0, strripos($CITY, '.'));
		//$sizeToday = count(unserialize(file_get_contents($file_path)));

		if (file_exists($file_path)) { // Если data файлик реально существует, обрабатываем его
			$current_row_array = unserialize(file_get_contents($file_path)); // Массив текущего магазина/города
