			...
			if ($tempHeap) {
				$is_heap = 1;
			 	foreach ($tempHeap as $key => $value) {
			 		if ($key == 'dns-shop.ru' || $key == 'technopoint.ru' || $key == 'frau-technica.ru') {
...
// Код / Город / Сайт / Дата = РИЦ | Цена магазина | url | Name
	...
	$objWriter->save('/var/www/reporter/reports/big/Monitoring_Vitek_Maxwell_usolie-sibirskoe_'.$date_for_name.'_'.$argv[2].'.xlsx');
	//Monitoring_Vitek_RU_060617_1
echo 'Sending...';
$fileName = 'Monitoring_Vitek_Maxwell_usolie-sibirskoe_'.$date_for_name.'_'.$argv[2].'.xlsx';
