 ...
 */
	...
	foreach ($arrayOfDates as $date) {
			 	...
			 	foreach ($tempHeap as $key => $value) {
			 		foreach ($value as $key2 => $value2) {
			 			if ($key2 != 'Усолье-Сибирское' ) {// USOLIE SIBIRSKOE !!! KOSTYL !!!
			 				if ($key2 == 'Якутск' && $key == 'shop.v-lazer.com') { // Временный костыль - на месяц 27.11.17
								// echo 'Убрать этот костыль 15 декабря'
							} else {
				 			foreach ($value2 as $info) {
				 					$itemsArray[$info[1]][$key2][$key][$date] = array($info[4], $info[5], $info[2], $info[3], $date_group_index, 'VITEK', 'extra'); 
			...
			echo 'Дата: '.$date.PHP_EOL;
			$heap_file_path = 'http://attraktor:eldorado@82.193.102.178:2080/vitek/heap/'.$date.'_extra.data';
			//$heap_file_path = '/var/www/'.$argv[2].'/heap/'.$date.'_1'.'.data';
				...
				$is_heap = 1;
			 	foreach ($tempHeap as $key => $value) {
			 		foreach ($value as $key2 => $value2) {
			 			if ($key2 != 'Усолье-Сибирское' ) {// USOLIE SIBIRSKOE !!! KOSTYL !!!
				 			foreach ($value2 as $info) {
				 					$itemsArray[$info[1]][$key2][$key][$date] = array($info[4], $info[5], $info[2], $info[3], $date_group_index, 'MAXWELL', 'extra'); 
				 				// КОД -> Город -> Сайт -> Дата  =  РИЦ | Цена магазина | url | Name | Индекс даты
							}
						}
			 		}
			...
			echo 'Дата: '.$date.PHP_EOL;
			$heap_file_path = 'http://attraktor:eldorado@82.193.102.178:2080/maxwell/heap/'.$date.'_extra.data';
			//$heap_file_path = '/var/www/'.$argv[2].'/heap/'.$date.'_1'.'.data';
	...
	$objPHPExcel->getActiveSheet()->setTitle('Monitoring_VM_'.$date_for_name.'_'.$argv[2]);
	...
	$objPHPExcel->getActiveSheet()->getColumnDimension('C')->setWidth(7);			// Эксклюзив
	$objPHPExcel->getActiveSheet()->getColumnDimension('D')->setWidth(13);  	// Артикул
	$objPHPExcel->getActiveSheet()->getColumnDimension('E')->setWidth(29);		// Номенклатура
	$objPHPExcel->getActiveSheet()->getColumnDimension('F')->setWidth(7);			// Ключевые города
	$objPHPExcel->getActiveSheet()->getColumnDimension('G')->setWidth(18);		// Город
	$objPHPExcel->getActiveSheet()->getColumnDimension('H')->setWidth(18);		// Клиент
	$objPHPExcel->getActiveSheet()->getColumnDimension('I')->setWidth(6);			// РИЦ
...
/**/
	...
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2, 5, 'ЭКСКЛ');
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3, 5, 'Артикул');
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(4, 5, 'Номенклатура');
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(5, 5, 'Ключевые города');
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(6, 5, 'Город');
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(7, 5, 'Клиент');
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(8, 5, 'РИЦ');
...
/**
  ...
  */ 
	...
	$objPHPExcel->getActiveSheet()->getStyle('B5:Z5')->getAlignment()->setWrapText(true);
	$objPHPExcel->getActiveSheet()->getStyle('B5:Z5')->applyFromArray(
...
// Код / Город / Сайт / Дата = РИЦ | Цена магазина | url | Name
		...
		foreach ($siteArray as $city => $cityArray) { // Город
			foreach ($cityArray as $site => $dateArray) { // Сайт
				foreach ($dateArray as $date => $info) { // Дата и информация
					...
					$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($info[4]+10, 5, $dateValue); // Дата
					$objPHPExcel->getActiveSheet()
							    ->getStyle($objPHPExcel->getActiveSheet()->getCellByColumnAndRow($info[4]+10, 5)->getCoordinate())
							    ->getNumberFormat()
					...
					if ($info[6] == 'extra') {
						$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2, $i, 'ЭКСКЛ'); 		// Эксклюзив
					}

					$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3, $i, $code);					// Артикул
					$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(4, $i, $info[3]);			// Номенклатура
					if (in_array($city, $important_cities)) {
						$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(5, $i, 'да');				// Ключевые города
					} else {
						$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(5, $i, '--');
					}
					$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(6, $i, $city);					// Город
					$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(7, $i, $site);					// Клиент
					$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(8, $i, $info[0]); 			// РИЦ
					//$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(7, $i, $info[2]); 		// Зона цены
					//////////
					$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($info[4]+10, $i, $info[1]); // Цена магазина
					...
					*/
					...
					if ($color) {
						$colorCell = $objPHPExcel->getActiveSheet()->getCellByColumnAndRow($info[4]+10, $i)->getCoordinate();
	...
	$objPHPExcel->getActiveSheet()->getStyle('I5:'.$endC)->applyFromArray(array( 'font'  		=> array('color' => array('rgb' => 'FF0000'))));
...
curl_setopt($ch, CURLOPT_PROXYUSERPWD, 'gp3071146:d2ClGk03L6');
...
if ($argv[1] == 'vitek_big_vm&extra') { // Если отчёт Витек, то высылаем "письмо счастья"
	// ADD THIS:
	$email = new PHPMailer\PHPMailer\PHPMailer();
	$email->IsSMTP();
	$email->CharSet = 'utf-8';
	...
	$email->AddEmbeddedImage('/var/www/reporter/settings/logo.png', 'pricinglogix-logo', 'logo.png');
		...
		$email->Subject   = 'Monitoring_Vitek-Rondell-Maxwell_'.$date_for_name.'_'.$argv[2];
