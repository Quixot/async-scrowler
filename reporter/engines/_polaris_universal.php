<?php
/*
$is_obolon_ok = file_get_contents('/var/www/lib/is_obolon_ok.txt');

if ($is_obolon_ok != 0) { // Включаться только если PL1 в ауте
	die();
}
*/
/**
	...
	foreach ($priceArray as $names => $pos_code_price) {
		...
		if ($pos_code_price[6]) {
			$objPHPExcel->getActiveSheet()->getStyle('G'.$currpos)->applyFromArray(
					array('fill' => array('type'  => PHPExcel_Style_Fill::FILL_SOLID, 'color' => array('rgb' => 'fcd5b4'))));
		...
		// Если core ассортимент, закрашиваем модель в #92d050
		if (in_array($model, $core_assort)) {
			$objPHPExcel->getActiveSheet()->getStyle('F'.$currpos)->applyFromArray(
					array('fill' => array('type'  => PHPExcel_Style_Fill::FILL_SOLID, 'color' => array('rgb' => '92d050'))));
		}
