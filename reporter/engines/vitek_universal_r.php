 ...
 */
$core_assort = array(
	'456 Набор ножей на подставке Spalt',
	'Ковш с/кр 18 см 1,6 л Zeita RDA-122',
	'382 Набор 6 предметов (кастрюли с/кр 20, 24 и молочник с/кр 16 см) Symphonia',
	'735 Ковш с кр. 16см 1,7 л Bojole Rondell',
	'736 Кастрюля с кр. 20см 3,3 л Bojole Rondell',
	'737 Кастрюля с кр. 24см 5,6 л Bojole Rondell',
	'389 Набор посуды 8 предметов Creative Rondell',
	'Кастрюля с кр. 18 см (2,4л) Charm RDS-732',
	'731 Ковш 16 см (1,5л) Charm Rondell',
	'Кастрюля с кр. 20 см (3,9л) Charm RDS-733',
	);
 ...
 */
    ...
    array(
		...
		// Если core ассортимент, закрашиваем модель в #92cddc
		if (in_array(trim($names), $core_assort)) {
			$objPHPExcel->getActiveSheet()->getStyle('B'.$currpos)->applyFromArray(array('fill' => array('type'  => PHPExcel_Style_Fill::FILL_SOLID, 'color' => array('rgb' => '92cddc'))));
			$objPHPExcel->getActiveSheet()->getStyle('C'.$currpos)->applyFromArray(array('fill' => array('type'  => PHPExcel_Style_Fill::FILL_SOLID, 'color' => array('rgb' => '92cddc'))));
		}
 ...
 */
	...
	foreach ($CITY_ORDER as $store_temp => $city_temp) {
		...
		foreach ($city_temp as $key => $cont) {
			if ($cont && $store_temp !== 'Усолье-Сибирское') {
				echo ' '.$key.PHP_EOL;
				// Шапочка
 ...
 */
	...
	foreach ($CITY_ORDER as $store_temp => $city_temp) {
		...
		echo ' '.$store_temp.PHP_EOL;
		if ($store_temp == 'Усолье-Сибирское') {
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($iRow, 2, $store_temp);
			$store_start = $objPHPExcel->getActiveSheet()->getCellByColumnAndRow($iRow, 2)->getCoordinate();
			$objPHPExcel->getActiveSheet()->getStyle($store_start)->applyFromArray(array(
					'alignment' => array(
 ...
 */
	...
	$istatus = 1;
	foreach ($STORE_ORDER as $store_temp => $city_temp) {
		if ($store_temp) { // Если в городе есть информация
	...
	$objWriter->save('/var/www/reporter/reports/'.date('d.m.y').'/'.$fileName.'.usolie-sibirskoe.xlsx');
	echo "Final: ".memory_get_usage()." bytes \n";
	echo "Peak: ".memory_get_peak_usage()." bytes \n";
