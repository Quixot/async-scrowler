 ...
 */
	$pathes = glob('/var/www/polaris/engines/*/data/'.$date.'*polaris*.data');
	...
	foreach ($pathes as $keypath) { // Перебираем каждый файлик
		...
		if (file_exists($keypath) && $CITY == 'moscow') { // Если data файлик реально существует, обрабатываем его
			...
			foreach ($current_row_array as $URL => $content) { // Перебираем позицию за позицией и ищем соответствия с эталонным массивом
				...
				if ($key !== false && $content[1] > 0) { // Ищем по URL или по NAME
					...
					*/
					...
					if ($URL && $CODE && $PRICE && $STORE /*$REALPRICE && $PRICE && isset($STORE_ORDER[$STORE][$CITY_RU])*/) {
