 ...
 */
	...
	foreach ($pathes as $key) { // Перебираем каждый файлик
		...
		if (file_exists($key) && (strripos($STORE, 'dns-shop.ru') !== false || strripos($STORE, 'technopoint.ru') !== false || strripos($STORE, 'frau-technica.ru') !== false /**/)) { // Если data файлик реально существует, обрабатываем его
 ...
 */
	...
	// Имя книги
	$objPHPExcel->getActiveSheet()->setTitle('Result_City->_Shop_'.$cur_date_sheet.'_0');
	// Ширины столбцов 
 ...
 */
	...
	$objSKU = PHPExcel_IOFactory::load('/var/www/reporter/reports/Report_SKU_Rondell_RU_small.xlsx');
	if (1 == 2) {
...
/**
 * END Стилизация
 * sudo php app.php rondell_night rondell_night rondell Report_Rondell_RU_$(date +\%d\%m\%y)_0
 */
	...
	$objWriter->save('/var/www/reporter/reports/Report_SKU_Rondell_RU_'.date('d.m.y').'_small.xlsx'); 

	echo copy(
		'/var/www/reporter/reports/Report_SKU_Rondell_RU_'.date('d.m.y').'_small.xlsx',
		'/var/www/reporter/reports/Report_SKU_Rondell_RU_small.xlsx');
