 ...
 */
	...
	$present_file = file_get_contents('/var/www/polaris.ua/heap/'.date('d.m.y').'_'.$argv[3].'.data');
	...
	$objPHPExcel = PHPExcel_IOFactory::load('/var/www/reporter/settings/polaris.ua_common.xlsx');
/**
 ...
 */
	...
	foreach ($arrayOfDates as $date) {
		//if (strripos($date, '05.17') === false) { // Убираем май !!! KOSTYL !!!
			...
			$heap_file_path = '/var/www/polaris.ua/heap/'.$date.'_'.$argv[3].'.data';
			 		...
			 		foreach ($value as $key2 => $value2) {
				 		foreach ($value2 as $info) {
				 			if ($info && $info[5]/$info[4] >= 0.33 && $info[5]/$info[4] <= 3) { // Отсечка уценки или ошибки с ценой - сильно завышена, сильно занижена
				 				...
				 				if (in_array($key, $sites_array)) { // Костыль - убираем лишние магазины:
