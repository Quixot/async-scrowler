 ...
 */
	...
	foreach ($arrayOfDates as $date) {
		//if (strripos($date, '05.17') === false) { // Убираем май !!! KOSTYL !!!
			...
			$files = glob('/var/www/'.$argv[2].'/heap/'.$date.'_*.data');

			//print_r($files);
			//die();

			foreach ($files as $heap_file_path) {

				$tempHeap = unserialize(file_get_contents($heap_file_path));
				echo $heap_file_path.PHP_EOL;
				echo date("H:i:s", filectime($heap_file_path)).PHP_EOL;
				//die();
				//file_put_contents('/var/www/reporter/reports/polaris_big/1.txt', print_r($tempHeap, 1)); die();
				// Дата
				$dateValue = $datedmy;
				if ($tempHeap) {
				 		...
				 		foreach ($value as $key2 => $value2) {
					 		foreach ($value2 as $info) {
					 			if ($info && $info[5]/$info[4] >= 0.33 && $info[5]/$info[4] <= 3) { // Отсечка уценки или ошибки с ценой - сильно завышена, сильно занижена
					 				$itemsArray[$info[1]][$key2][$key][$date.' '.date("H:i:s", filectime($heap_file_path))] = array($info[4], $info[5], $info[2], $info[3], $date_group_index, $info[6]); 
	...
	$objPHPExcel->getActiveSheet()->getColumnDimension('G')->setWidth(11);		// Merchant
...
/**
  ...
  */ 
	...
	echo 'Height'.PHP_EOL;
	echo microtime(1)-$timer.PHP_EOL;	
	$objPHPExcel->getActiveSheet()->getRowDimension(5)->setRowHeight(47);

	$objPHPExcel->getActiveSheet()->getStyle('B5:CY5')->getAlignment()->setWrapText(true);
	$objPHPExcel->getActiveSheet()->getStyle('B5:CY5')->applyFromArray(
		...
		foreach ($siteArray as $city => $cityArray) { // Город
			foreach ($cityArray as $site => $dateArray) { // Сайт
				...
				foreach ($dateArray as $date => $info) { // Дата и информация
					...
					//$dateValue = PHPExcel_Shared_Date::PHPToExcel(DateTime::createFromFormat('d.m.y H:i:s', $date));
					//$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($info[4]+8, 5, $dateValue); // Дата
					$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($info[4]+8, 5, $date); // Дата
 ...
 */
	...
	$fileName = 'Monitoring_'.ucfirst($argv[2]).'_'.$date_for_name.'_all.xlsx';
