...
/*
	switch (date('H')) {
		case '8': $sheet_name_index = '1';
			break;		
		case '9': $sheet_name_index = '1';
			break;
		case '12': $sheet_name_index = '2';
			break;
		case '14': $sheet_name_index = '2';
			break;
		case '15': $sheet_name_index = '2';
			break;
		default:
			$sheet_name_index = '1';
			break;
 ...
 */
	...
	$objPHPExcel->getActiveSheet()->getColumnDimension('H')->setWidth(6);  		// shop-polaris.ru цены
	// Высоты столбцов
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('H3', 'цена shop-polaris.ru');
	...
	$objPHPExcel->getActiveSheet()->getStyle('H3')->applyFromArray(array( 'font'  		=> array('color' => array('rgb' => '000000')),
																																				'alignment' => array('horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_LEFT,
																																														 'vertical' 	=> PHPExcel_Style_Alignment::VERTICAL_BOTTOM)));
	...
	$objPHPExcel->getActiveSheet()->getStyle('H3')->getAlignment()->setTextRotation(90);
	...
	foreach ($priceArray as $names => $pos_code_price) {
		...
		if ($shop_polaris_prices[$pos_code_price[1]]) {
			$objPHPExcel->getActiveSheet()->SetCellValue('H'.$currpos, $shop_polaris_prices[$pos_code_price[1]]); // shop-polaris.ru price
		} else {
			$objPHPExcel->getActiveSheet()->SetCellValue('H'.$currpos, $shop_polaris_prices_by_name[$model]); // shop-polaris.ru price
		}
		...
		$styleArray = array('font'  		=> array('color' => array('rgb' => '000000'),),
									    	'alignment' => array('horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_LEFT, 'vertical' => PHPExcel_Style_Alignment::VERTICAL_BOTTOM));
		$objPHPExcel->getActiveSheet()->getStyle('H'.$currpos)->applyFromArray($styleArray);
		$objPHPExcel->getActiveSheet()->getStyle("B3:H3")->applyFromArray(
    array(
        'borders' => array(
            'allborders' => array(
 ...
 */
	...
	// Цвет границ шапки
	$objPHPExcel->getActiveSheet()->getStyle('I2:'.$objPHPExcel->getActiveSheet()->getCellByColumnAndRow($iRow-1, 3)->getCoordinate())->applyFromArray(
    array(
        'borders' => array(
            'allborders' => array(
                'style' => PHPExcel_Style_Border::BORDER_THIN,
 ...
 */
	...
	file_put_contents('/var/www/polaris/heap/'.date('d.m.y').'_'.$sheet_name_index.'.data', serialize($STORE_ORDER));
