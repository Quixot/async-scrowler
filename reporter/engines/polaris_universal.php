 ...
 */
	...
	foreach ($pathes as $keypath) { // Перебираем каждый файлик
		...
		if (file_exists($keypath)) { // Если data файлик реально существует, обрабатываем его
			...
			// Массив цен, чтобы сравнивать, была ли уже цена
			$check_small_price_array = array();
				...
				if ($key !== false && $content[1] > 0) { // Ищем по URL или по NAME
					...
					*/
					...
					if ($URL && $CODE && $REALNAME && /*$REALPRICE &&*/ $PRICE && isset($STORE_ORDER[$STORE][$CITY_RU])) {
						 ...
						 */
						if (($PRICE/$REALPRICE >= 0.4 && $PRICE/$REALPRICE <= 2.5) || $STORE == 'wildberries.ru') { // Проверяем не сильно ли завышена/занижена цена
							if (!$check_small_price_array[$CODE] || $check_small_price_array[$CODE] > $PRICE) { // Если цены ещё не было или она та что была больше текущей
								//																										код    урл   клиент     РИЦ         магаз   
								...
								$check_small_price_array[$CODE] = $PRICE;
								/*
								echo $REALNAME.PHP_EOL;
								echo $REALPRICE.PHP_EOL;
								echo $PRICE.PHP_EOL;
								echo $PRICE/$REALPRICE.PHP_EOL;
								echo $STORE.PHP_EOL;
								echo $CITY_RU.PHP_EOL;
								*/
			...
			if ($cont) {
				...
				foreach ($cont as $value) {
					if ($value && $value[4] == 0) { // Если РИЦ == 0 то просто выводим цену, не закрашиваем
						...
						if (($value && $value[5]/$value[4] >= 0.4 && $value[5]/$value[4] <= 2.5) || $key == 'wildberries.ru') { // 
							//echo '■';
							//echo $store_temp.PHP_EOL;
							//echo $key.PHP_EOL;
							//print_r($value);
			...
			if ($cont) {
				...
				foreach ($cont as $value) { // Перебираем позиции внутри магазина
				    ...
				    $objPHPExcel->getActiveSheet()->getStyle($colorCell)->applyFromArray($styleArray);
						...
						if (($value && $value[5]/$value[4] >= 0.4 && $value[5]/$value[4] <= 2.5) || $store_temp == 'wildberries.ru') { // Если цены выша или ниже на 100%, определяем как ошибку
