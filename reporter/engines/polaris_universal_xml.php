		...
		if (file_exists($keypath)) { // Если data файлик реально существует, обрабатываем его
			...
			foreach ($current_row_array as $URL => $content) { // Перебираем позицию за позицией и ищем соответствия с эталонным массивом
				...
				if ($key !== false && $content[1] > 0) { // Ищем по URL или по NAME
					...
					$CLIENT_NAME = $priceArray[$REALNAME][7];
					/*
					echo '*******'.PHP_EOL;
 ...
 */
	...
	$shop->appendChild($xml->createElement('parser_end_time', date("d.m.y H:i:s", filemtime($heap_file_path))));
	...
	if ($tempHeap) { // Пересобираем HEAP для удобства поиска
		...
		//die('No heap file!');
		mail('alexandr.volkoff@gmail.com', 'Polaris XML Problem!', 'Can\'t make xml!', "From: info@pricinglogix.com");
	}
	...
	foreach ($client_xml[shop]->offers->offer as $val) { // Основной цикл на базе файлика клиента
		...
		if (stripos($itemsArray[$model], $model) !== false) {
			echo $val->model.PHP_EOL;
			echo $val->artikul.PHP_EOL.PHP_EOL;
			print_r($itemsArray[$model]); 
			die();
		}
