 ...
 */
	...
	$from = new DateTime('now -15 day'); // -15
	...
	foreach ($arrayOfDates as $date) {
		//if (strripos($date, '05.17') === false) { // Убираем май !!! KOSTYL !!!
			...
			//$heap_file_path = 'http://attraktor:eldorado@'.($argv[2]=='rondell'?'176.36.102.174:3080/':'82.193.102.178:2080/').$argv[2].'/heap/'.$date.'_1'.'.data';
	...
	$objPHPExcel->getActiveSheet()->getColumnDimension('B')->setWidth(13);  	// Артикул
	$objPHPExcel->getActiveSheet()->getColumnDimension('C')->setWidth(29);		// Номенклатура
	$objPHPExcel->getActiveSheet()->getColumnDimension('D')->setWidth(7);			// Ключевые города
...
/**/
	...
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3, 5, 'Ключевые города');
...
// Код / Город / Сайт / Дата = РИЦ | Цена магазина | url | Name
		...
		foreach ($siteArray as $city => $cityArray) { // Город
			foreach ($cityArray as $site => $dateArray) { // Сайт
				foreach ($dateArray as $date => $info) { // Дата и информация
					...
					$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2, $i, $info[3]);			// Номенклатура
					if (in_array($city, $important_cities)) {
						$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3, $i, 'да');				// Ключевые города
					} else {
						$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3, $i, '--');
					}
					...
					*/
					...
					if ($color) { // Цвет Зона цены
						$colorCell = $objPHPExcel->getActiveSheet()->getCellByColumnAndRow($info[4]+6, $i)->getCoordinate();
	...
	$objWriter->save('/var/www/reporter/reports/big/Monitoring_'.ucfirst($argv[2]).'_usolie-sibirskoe_'.$date_for_name.'_'.$argv[3].'.xlsx');
	//Monitoring_Vitek_RU_060617_1
echo 'Sending...';
$fileName = 'Monitoring_'.ucfirst($argv[2]).'_usolie-sibirskoe_'.$date_for_name.'_'.$argv[3].'.xlsx';
