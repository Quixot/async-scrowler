 ...
 */
	...
	foreach ($client_xml[shop]->offers->offer as $val) { // Основной цикл на базе файлика клиента
		...
		$price_club = trim($val->price_club);
		...
		if (!in_array($artroz, $except_arts)) {
			 ...
			 */
				...
				$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(7, $i, trim($val->price_club));		// price club
			/**
			 * EXCEL INJECTION END
			 */
				...
				if ($itemsArray[$code]) { // Если позиция есть в парсинге
					foreach ($itemsArray[$code] as $cl_name => $cl_value) {
						//if ($cl_value[1]/trim($val->price) >= 0.5 && $cl_value[1]/trim($val->price) <= 2) { // Убираем цены значительно ниже и выше РИЦ
